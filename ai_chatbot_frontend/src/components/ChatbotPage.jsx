...
const ChatbotPage = () => {
  ...
  useEffect(() => {
    ...
    if (storedSession && storedChat) {
     ...
     axios
         .post(`${BASE_URL}/api/login`, { email: "user@example.com", password: "yourpassword" })
        .then((res) => {
  ...
  const storedSession = sessionStorage.getItem("chatSessionId");
  try {
    const res = await axios.post(`${BASE_URL}/api/chat/new`, null, {
      params: { session_id: storedSession },
    });
